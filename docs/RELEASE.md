...
`https://github.com/langley-2/mvam-v1/releases/download/v1.0.2/Taffy-Setup-x64.exe`

The current release is a pre-release, so GitHub's `/releases/latest/` alias does not select it. A private repository’s assets require access. Use a separate public downloads repository if the source should stay private.
...
5. Set `package.json` and lockfile version together, create the matching `v...` Git tag, and push it. The workflow publishes a **pre-release** containing both installers and SHA-256 checksums. Promote it to a stable release only after the installation and launch checks above pass.
